...
- `apps/web/components/MarketMap.tsx` — the `#market-map-canvas` div the job screenshots (just the treemap, not the filter sidebar/PSEi summary bar/top movers — don't remove/rename that id without updating the job). `apps/web/app/page.tsx`'s `#market-map-capture` wrapper is unrelated now (a11y skip-link target only).
