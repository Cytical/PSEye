...
describe("transactionsToCsv", () => {
  it("serializes transactions with a header row", () => {
    const transactions: Transaction[] = [
      { id: "1", ticker: "BDO", type: "buy", shares: 100, price: 120.5, date: "2026-01-01" },
    ];
    expect(transactionsToCsv(transactions)).toBe("ticker,type,shares,price,date\nBDO,buy,100,120.5,2026-01-01");
  });
});
...
describe("parseTransactionsCsv", () => {
  it("round-trips what transactionsToCsv produces", () => {
    ...
    expect(parsed).toEqual([
      { ticker: "BDO", type: "buy", shares: 100, price: 120.5, date: "2026-01-01" },
      { ticker: "SM", type: "sell", shares: 10, price: 500, date: "2026-02-01" },
    ]);
  });

  it("skips blank lines and a header row without failing", () => {
    const csv = "ticker,type,shares,price,date\n\nBDO,buy,10,100,2026-01-01\n";
